...
def get_items(pos_profile, price_list=None):
    ...
    if items_data:
        ...
        for item in items_data:
            ...
            ivr = 0
            if frappe.db.exists('Bin', {'item_code':item_code}):
                item_bin = frappe.get_last_doc("Bin", {'item_code':item_code})
                ivr = item_bin.valuation_rate
            if pos_profile.get("posa_display_items_in_stock") and (
                ...
                row.update(item)
                lbr = 0
                if frappe.db.exists("Item Price", {"item_code":row.get("item_code"), "buying":True}):               
                    lbr = frappe.get_last_doc("Item Price", {"item_code":row.get("item_code"), "buying":True}).price_list_rate 
                row.update(
                    {
                        ...
                        "item_purchase_rate": lbr if lbr else 0.00,
                        "item_valuation_rate":ivr or 0.00
                    }
                )
                result.append(row)
    return result
...
@frappe.whitelist()
def get_last_customer_rate(customer, item_code):
    lsr = frappe.db.sql("""
        SELECT
            sii.item_code,
            si.customer, 
        ...
        ON 
            si.name = sii.parent
        WHERE 
            sii.item_code='{item_code}' AND
            si.customer = '{customer}' AND
            si.docstatus = 1
        ORDER BY
            sii.creation DESC
    """.format(item_code=item_code, customer=customer), as_dict=True)
    if lsr:
        return lsr[0].rate
    else:
        return 0
