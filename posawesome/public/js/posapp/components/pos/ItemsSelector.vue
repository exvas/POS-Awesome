  ...
  methods: {
    ...
    getItmesHeaders() {
      const items_headers = [
        ...
        { text: __('OEM'), value: 'oem_part_number', align: 'start' },
      ...
      if (!this.pos_profile.display_oem_part_number) {
        items_headers.splice(2, 2);
      }
    ...
    filtred_items() {
      ...
      if (!this.search || this.search.length < 3) {
        ...
        if (filtred_list.length == 0) {
          filtred_list = (filtred_group_list.filter((item) =>{
            var check = 0
            if(item.item_code.toLowerCase().includes(this.search.toLowerCase())){
              check+=1
            }
            else if(item.item_name.toLowerCase().includes(this.search.toLowerCase())){
              check+=1
            }
            else if(item.oem_part_number){
              if(item.oem_part_number.toLowerCase().includes(this.search.toLowerCase())){
                check+=1
              }
            }
            if(check>0){
