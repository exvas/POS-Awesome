            ...
            <template v-slot:expanded-item="{ headers, item }">
              <td :colspan="headers.length" class="ma-0 pa-0">
                ...
                <v-row class="ma-0 pa-0">
                  ...
                  <!-- Custom field Purchase Rate -->
                  <v-col cols="4">
                    <v-text-field
                      dense
                      outlined
                      color="primary"
                      :label="frappe._('Purchase Rate')"
                      background-color="white"
                      hide-details
                      v-model="item.item_purchase_rate"
                      disabled
                    ></v-text-field>
                  </v-col>
                  <!-- Custom field Last Purchase Rate -->
                  <v-col cols="4">
                    <v-text-field
                      dense
                      outlined
                      color="primary"
                      :label="frappe._('Last Purchase Rate')"
                      background-color="white"
                      hide-details
                      :value="formtCurrency(item.item_last_purchase_rate)"
                      disabled
                    ></v-text-field>
                  </v-col>
                  <!-- Custom field Valuation Rate -->
                  <v-col cols="4">
                    <v-text-field
                      dense
                      outlined
                      ...
                      background-color="white"
                      hide-details
                      :value="formtCurrency(item.item_valuation_rate)"
                      disabled
                    ></v-text-field>
                  </v-col>
                  <!-- Custom field previous item rate for customer -->
                  <v-col cols="4">
                    <v-text-field
                      dense
                      outlined
                      color="primary"
                      :label="frappe._('Previous Customer Rate')"
                      background-color="white"
                      hide-details
                      :value="prevCustomerRate(item)"
                      disabled
                    ></v-text-field>
                  </v-col>
...
export default {
  ...
  methods: {
    prevCustomerRate(item){
      if(item.last_customer_rate){
        return item.last_customer_rate
      }
      else if(this.customer && item.item_code && !item.last_customer_rate){
        frappe.xcall("posawesome.posawesome.api.posapp.get_last_customer_rate", {
          customer:this.customer,
          item_code: item.item_code
        }).then((lcr)=>{
          item.last_customer_rate = lcr
          return lcr
        })
      }
    },
    ...
    get_invoice_items() {
      ...
      this.items.forEach((item) => {
        const new_item = {
          ...
          item_last_purchase_rate: item.item_last_purchase_rate,
          item_purchase_rate: item.item_purchase_rate,
          item_valuation_rate: item.item_valuation_rate,
          last_customer_rate: item.last_customer_rate
        };
